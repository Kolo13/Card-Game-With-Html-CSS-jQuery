  ...
  function CardGame(numPlayers) {
    ...
    this.play = function() {   
      ...
      for (i = 0; i < this.player.length; i++) {
        //Gets each players top card
        this.player[i].topCardRank = this.player[i].pile[this.player[i].pile.length - 1].rank
        //Displays each players top card
        //console.log(this.player[i].name + " has the " + this.player[i].pile[this.player[i].pile.length - 1].show());
  ...
  highCardGame = new CardGame(prompt("Welcome to High Card.  Select the number of players"));
  highCardGame.init();
